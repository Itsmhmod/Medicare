...
@section('content')
    ...
    <div class="patients-container p-3">
        ...
        <div class="d-flex justify-content-between align-items-center mb-3">
            {{-- <a href="{{ route('schedule.create') }}" class="btn btn-primary ms-3"> <!-- Left margin for Add button -->
                Add
            ...
            <table id="myTable" class="table table-striped table-bordered text-center mx-3"> <!-- Margin added -->
                <thead>
                    <tr>
                        ...
                        <th><strong>Visita_No</strong></th>
                        <th><strong>Days</strong></th>
                        <th><strong>Doctor</strong></th>
                        <th><strong>Mofify</strong></th>
                    </tr>
                </thead>
                <tbody>
                    @foreach ($bookAppointments as $bookAppointment)
                        <tr>
                            <td><strong>{{ $bookAppointment->id }}</strong></td>
                            <td><strong>{{ $bookAppointment->name }}</strong></td>
                            <td><strong>{{ $bookAppointment->phone }}</strong></td>
                            <td><strong>{{ $bookAppointment->email }}</strong></td>
                            <td><strong>{{ $bookAppointment->date }}</strong></td>
                            <td><strong>{{ $bookAppointment->visita_no }}</strong></td>
                            <td><strong>{{ $bookAppointment->days }}</strong></td>
                            <td><strong>{{ $bookAppointment->doctor->name }}</strong></td>
                            <td>
                                {{-- <a href="{{ route('schedule.edit', $schedule->id) }}" class="btn btn-primary btn-sm p-2 mx-2">
                                ...
                                <a href="javascript:;" class="btn btn-primary btn-sm p-2 mx-2" data-bs-toggle="modal"
                                    data-bs-target="#deleteSliderModal{{ $bookAppointment->id }}">
        ...
        @include('Doctor.BookAppointment.delete')
